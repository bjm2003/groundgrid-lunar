...
接手基线：`main` / `14e0fde`；当前工作分支：`codex/p0-trajectory-fixes`（尚未合并）。最近收到的 Ubuntu 短诊断来自 `878cbdf`，直线、转弯和困难目标各通过一次；该版本的完整巡回与正式五场景基线仍待验证。
...
三次 `test_closed_loop` 均通过了双方完成状态、无碰撞断言和无陈旧停车断言，但没有回传完整安全距离统计与 XML；`test_metrics_over_trials` 在这些短诊断模式下被跳过。当前只能记录“三项单次短闭环通过”，不能声称完整包测试、99% 成功率或五场景验收通过。下一步保持 `878cbdf` 运行代码不变，执行完整 `mixed / n_trials=3` 包回归，保存 JSON、XML 和完整日志。
