...
// may be republished, while an interrupted route is only history for checked back-out.
// Replaying that history as a new route would reset the follower to its already completed
// first rotation/cusp. Keep publication continuity explicit instead of inferring it from
...
public:
    ...
    // preserving the historical route for the separately collision-checked back-out rung.
